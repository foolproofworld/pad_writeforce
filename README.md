...
- **本质限制**：`adb push` 依赖设备端 `sync` 服务，很多厂商固件只允许单路会话串行处理，导致即便主机并发发起多条 push，设备端仍按顺序写，无法真正叠加带宽。
- **有限的缓解手段**（需按设备特性评估）：
  - 尝试同时保持 **USB + Wi‑Fi** 两条 ADB 连接（`adb tcpip 5555` 后再 `adb connect <设备IP>`），并在脚本里固定 `device_id` 分别跑两份实例，对同一设备形成两条独立传输链路；是否能并行取决于设备端实现。
  - 在可 root 或可执行持久 shell 的环境下，改用 `adb shell` 启动设备本地写入（例如多路 `dd if=/dev/zero of=/sdcard/... bs=1M count=... &`），由设备自身并行落盘，绕过主机到设备的单路 sync 限制。当前脚本聚焦主机推送，如需这种“本地自写”模式需手动执行或自行扩展。
  - 如果测试目标允许，也可以挂载多分区/多存储路径并各自跑独立实例，以验证不同挂载点的并发写入能力。
- **不可行的点**：单设备、单 sync 服务下无法通过增加线程或开多个 terminal 来“强行并行”，因为设备端会主动串行化；脚本已经在日志中记录并行尝试，但吞吐受限时说明设备栈没有开放并行。
